...
const JobEdit = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [branchesRes, categoriesRes, questionsRes, jobRes] = await Promise.all([
          getBranches(),
          getJobCategories(),
          getCustomQuestions(),
          getJobById(id),
        ...
        setForm({
          ...job,
          custom_question: job.custom_question ? job.custom_question.split(",").map(Number) : [],
          applicant: job.applicant || "",
          visibility: job.visibility || "all",
        });

        setSkills(job.skill ? job.skill.split(",") : []);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/recruitment/job"); // go back to list
    } catch (error) {
    ...
    <div className="container mt-4">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-success small mb-0">Dashboard &gt; Job &gt; Edit Job</p>
        </div>
      </div>
                  ...
                  <Col md={6}>
                    ...
                    {["gender", "dob", "country"].map((field) => (
                      <Form.Check
                        ...
                        checked={form.applicant?.includes(field)}
                      />
                    ))}
