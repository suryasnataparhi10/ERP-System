...
import { getPerformanceTypes } from "../../../services/performanceTypeService";
...
const CompetencyList = () => {
  ...
  const [performanceTypeMap, setPerformanceTypeMap] = useState({});
  ...
  const [isClosingModal, setIsClosingModal] = useState(false); // ✅ added for animation
  ...
  const loadCompetencies = async () => {
    try {
      const data = await getCompetencies(); // service returns data array
  ...
  const loadPerformanceTypes = async () => {
    try {
      const data = await getPerformanceTypes(); // service returns data array
      setPerformanceTypes(data);

      // Map for fast access
      const map = data.reduce((acc, pt) => {
        acc[pt.id] = pt.name;
        return acc;
      }, {});
      setPerformanceTypeMap(map);
    } catch (err) {
  ...
  return (
    <div className="bg-white p-3 rounded shadow-sm">
      ...
      <div className="d-flex justify-content-between mb-3">
        <h5 className="fw-bold mb-0">Manage Competencies</h5>
          ...
          <tbody>
            {currentEntries.length > 0 ? (
              currentEntries.map((item) => (
                <tr key={item.id}>
                  <td>{item.name}</td>
                  <td>{performanceTypeMap[item.type] || "N/A"}</td>
                  <td>
                    <Button
                      variant="info"
                      size="sm"
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>{editId ? "Edit" : "Create"} Competency</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          ...
          <Form.Group className="mb-3">
            <Form.Label>Performance Type</Form.Label>
            <Form.Select
              value={formData.type}
              onChange={(e) =>
